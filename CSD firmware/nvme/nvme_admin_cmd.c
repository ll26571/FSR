...
int handle_get_features(NVME_ADMIN_COMMAND *nvmeAdminCmd, NVME_COMPLETION *nvmeCPL, unsigned int cmdSlotTag)
{
	...
	switch(features.FID)
	{
		...
		case 0x14:  // flush half the pages
		{
			unsigned int radio = nvmeAdminCmd->dword11;
			if (radio >= 0 && radio <= 100) {
				unsigned int flushed = flush_buffer(radio);
				xil_printf("%d pages are moved to the tail of the FTL buffer.\r\n", flushed);				
			}
			else
				xil_printf("error, the radio should between 0 and 100.\r\n");
