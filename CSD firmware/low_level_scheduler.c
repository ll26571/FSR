...
int CheckSearchTaskConfigDMA(){
	if(check_auto_rx_dma_partial_done(searchTask->rxDmaTail, searchTask->rxDmaOverFlowCnt)){
		...
		if (searchTask->need_path_walk) {  // need path walk
			...
			unsigned int file_ino = f2fs_path_crawl(path, path_len);
			if (file_ino == 0){  // f2fs_path_crawl failed, terminate the task
				abort_task();
				xil_printf("[CheckSearchTaskConfigDMA] failed to find the ino of the file, this task is terminated.\r\n");
				return 0;
			}
		...
		// t_total = ((time_end_retrieve - time_start_search) * 1000000) / (COUNTS_PER_SECOND);
		// tUsed = ((time_end_retrieve - time_start_retrieve) * 1000000) / (COUNTS_PER_SECOND);
		// xil_printf("Total time: %d us. Time of retrieve:  %d us.\r\n", t_total, tUsed);
		// xil_printf("nat read hit: %d, nat read miss: %d, data read hit: %d, data read miss: %d.\r\n", nat_read_hit, nat_read_miss, data_read_hit, data_read_miss);
		// nat_read_hit = nat_read_miss = data_read_hit = data_read_miss = 0;
...
int PopFromReqQueue(int chNo, int wayNo)
{
	...
	if (request == LLSCommand_RxDMA)
	{
		...
		if (lpn * 4 == FS_OFFSET){  // hit f2fs_super_block
			f2fs_updateSB(pageDataAddr);
		}
		else if (lpn * 4 == (FS_OFFSET + 512) || lpn * 4 == (FS_OFFSET + 1024)){
			// hit cp
			// XTime start, end;
			// XTime_GetTime(&start);
			f2fs_updateCP(pageDataAddr);
			// XTime_GetTime(&end);
	...
	switch(dieStatusTable->dieStatusEntry[chNo][wayNo].dieStatus)
	{
		...
		case DS_EXE:
			if(reqStatus == RS_DONE)
			{
				...
				else if(reqQueue->reqEntry[front][chNo][wayNo].request == V2FCommand_ReadPageTransfer && reqQueue->reqEntry[front][chNo][wayNo].search)
				{
					// xil_printf("read data done.\r\n");
					searchInPage(reqQueue->reqEntry[front][chNo][wayNo].pageDataBuf, reqQueue->reqEntry[front][chNo][wayNo].searchPageIndex);
			...
			{
				if(retryLimitTable->retryLimitEntry[chNo][wayNo] > 0)  // if there's a retry, retry
				{
					retryLimitTable->retryLimitEntry[chNo][wayNo]--;  // retry--
				...
				else  // retry times are used up, move forward
				{
					front = rqPointer->rqPointerEntry[chNo][wayNo].front;
					completion = completeTable->completeEntry[chNo][wayNo];
...
/*
	依据此way firstQueue headEntry的request类型对此way重新标记;
	若firstQueue空,则参照secondQueue;
	若secondQueue也空,则将此way标记为idle
*/
...
int ExeLowLevelReqPerCh(int chNo, int firstQueue)  //执行此Channel的队列调度
{
	int wayNo, idleWay, nextWay, enable, reqStatus, statusOption;
	unsigned int readyBusy;

	if(wayPriorityTable->wayPriorityEntry[chNo].idleHead != 0xf)  //此Channel存在idleWay，遍历每个idleWay以确定是否仍idle
	{
		wayNo = wayPriorityTable->wayPriorityEntry[chNo].idleHead;  //取head idleWay
		idleWay = 0;

		while(wayNo != 0xf)  //开始遍历检查
		{
			...
			if(enable)  //Req not empty or SubReq not empty, 此way不再idle
			{
				//从链表中删除此way
				if((dieStatusTable->dieStatusEntry[chNo][wayNo].nextWay != 0xf) && (dieStatusTable->dieStatusEntry[chNo][wayNo].prevWay != 0xf))  //中间
				{
				...
				else if((dieStatusTable->dieStatusEntry[chNo][wayNo].nextWay == 0xf) && (dieStatusTable->dieStatusEntry[chNo][wayNo].prevWay != 0xf))  //最右
				{
				...
				else if((dieStatusTable->dieStatusEntry[chNo][wayNo].nextWay != 0xf) && (dieStatusTable->dieStatusEntry[chNo][wayNo].prevWay == 0xf))  //最左
				{
				...
				nextWay = dieStatusTable->dieStatusEntry[chNo][wayNo].nextWay;  //取下一个way暂存
				FindPriorityTable(chNo, wayNo, firstQueue);
				wayNo = nextWay;
			}
			else  //此way的两个队列都空,仍idle
			{
				idleWay++;
	...
	if(wayPriorityTable->wayPriorityEntry[chNo].statusReportHead != 0xf)  //此Channel存在statusReport way, 遍历以确定status
	{
		readyBusy = V2FReadyBusyAsync(chCtlReg[chNo]);
		wayNo = wayPriorityTable->wayPriorityEntry[chNo].statusReportHead;  //取head

		while(wayNo != 0xf)  //开始遍历
		{
			if ((readyBusy >> wayNo) & 1)
			{
				if(dieStatusTable->dieStatusEntry[chNo][wayNo].queueSelect == REQ_QUEUE)
				...
				if(reqStatus != RS_RUNNING)  //若队列状态不是Running
				{
					if((dieStatusTable->dieStatusEntry[chNo][wayNo].nextWay != 0xf) && (dieStatusTable->dieStatusEntry[chNo][wayNo].prevWay != 0xf))  //中间
					{
						dieStatusTable->dieStatusEntry[chNo][dieStatusTable->dieStatusEntry[chNo][wayNo].prevWay].nextWay = dieStatusTable->dieStatusEntry[chNo][wayNo].nextWay;
					...
					ExeLowLevelReqPerDie(chNo, wayNo, reqStatus);  //进入状态机调度
	...
	if(wayPriorityTable->wayPriorityEntry[chNo].nvmeDmaHead != 0xf)  //此Channel存在需要进行nvmeDma操作的way,检查操作是否完成
	{
		wayNo = wayPriorityTable->wayPriorityEntry[chNo].nvmeDmaHead;  //取head
		...
		while(wayNo != 0xf)  //开始遍历
		{
			...
			if(enable)
			{
				...
				nextWay = dieStatusTable->dieStatusEntry[chNo][wayNo].nextWay;
				FindPriorityTable(chNo, wayNo, firstQueue);  //重新确定此way属于哪个链表
				wayNo = nextWay;
			}
			else
				wayNo = dieStatusTable->dieStatusEntry[chNo][wayNo].nextWay;
